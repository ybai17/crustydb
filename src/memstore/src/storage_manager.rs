...
impl StorageTrait for StorageManager {
    ...
    fn new_test_sm() -> Self {
        StorageManager::new(PathBuf::new())
    }
        ...
        for result in rdr.records() {
            ...
            match result {
                Ok(rec) => {
                    ...
                    self.insert_value(container_id, tuple.to_bytes(), tid);
                    inserted_records += 1;
                }
                _ => {
...
mod tests {
    ...
    fn test_simple_iter() {
        ...
        let mut check_tuple: Tuple = serde_cbor::from_slice(&check_bytes.0).unwrap();
        assert_eq!(tuple_bytes, check_bytes.0);
        assert_eq!(tuple, check_tuple);
        check_bytes = iter.next().unwrap();
        check_tuple = serde_cbor::from_slice(&check_bytes.0).unwrap();
        assert_eq!(tuple_bytes2, check_bytes.0);
        assert_eq!(tuple2, check_tuple);
